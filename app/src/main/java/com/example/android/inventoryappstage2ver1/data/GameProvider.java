...
public class GameProvider extends ContentProvider {
    ...
    private Uri insertGame(Uri uri, ContentValues values) {
        ...
        String name = values.getAsString(GameEntry.COLUMN_NAME);
        if (name == null) {
            throw new IllegalArgumentException("Game requires a name");
        }

        // Check that the category is valid
        Integer category = values.getAsInteger(GameEntry.COLUMN_CATEGORY);
        if (category == null || !GameEntry.isValidCategory(category)) {
            throw new IllegalArgumentException("Game requires valid category");
        }

        // check if the provided price is greater than to 0
        Integer price = values.getAsInteger(GameEntry.COLUMN_PRICE);
    ...
    private int updateGame(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        // If the {@link GameEntry#COLUMN_NAME} key is present,
        // check that the name value is not null.
        if (values.containsKey(GameEntry.COLUMN_NAME)) {
            String name = values.getAsString(GameEntry.COLUMN_NAME);
            if (name == null) {
                throw new IllegalArgumentException("Game requires a name");
            }
        ...
        // If the {@link GameEntry#COLUMN_CATEGORY} key is present,
        // check that the gender value is valid.
        if (values.containsKey(GameEntry.COLUMN_CATEGORY)) {
            Integer category = values.getAsInteger(GameEntry.COLUMN_CATEGORY);
            if (category == null || !GameEntry.isValidCategory(category)) {
                throw new IllegalArgumentException("Game requires valid category");
            }
        ...
        // If the {@link GameEntry#COLUMN_PRICE} key is present,
        // check that the weight value is valid.
        if (values.containsKey(GameEntry.COLUMN_PRICE)) {
